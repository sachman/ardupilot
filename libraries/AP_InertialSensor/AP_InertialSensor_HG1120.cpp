...
uint8_t AP_InertialSensor_HG1120::_getIMUaddress() {
    ...
    if (_drdy_pin_num_xg >= 0) {
        ...
        if (DATA_RDY_LEVEL == hal.gpio->read(_drdy_pin_num_xg)) {
            ...
            honWl_IMU_spiFetch(imuMessage.raw_data);
        }
    }
    else {
        /* Fetch a data message from the sensor  */
    ...
    /* Parse the data */
   dataPacket_isValid = parse_HonWlIMU_data(imuMessage.raw_data);
...
bool AP_InertialSensor_HG1120::parse_HonWlIMU_data(uint8_t *data_in) {
   ...
   if (data_in[2] == CONTROL_MSG_ID) {
       ...
       for (i=0; i< 12; i++) {
           u16sum += ((imuMessage.spiCtrlMsgPacket.controlMessage.controlData.data[i][1] << 8) |
                   imuMessage.spiCtrlMsgPacket.controlMessage.controlData.data[i][0]);
       }
       /* Sanity check  */
...
void AP_InertialSensor_HG1120::start(void)
{
    ...
    _dev->register_periodic_callback(1000, FUNCTOR_BIND_MEMBER(&AP_InertialSensor_HG1120::_poll_data, void));
}

/**
 * Timer process to poll for new data from the HG1120.
 */
//bool AP_InertialSensor_HG1120::_poll_data() {
...
// */
//void AP_InertialSensor_HG1120::_read_data_transaction_g(uint16_t samples)
//{
//    struct sensor_raw_data raw_data = { };
